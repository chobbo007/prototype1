...
def generate_final_report(team_history, filename, ai_text="분석 데이터를 생성중입니다..."):
    """
    ...
    *주의: 프레임별로 팀원들의 위치가 저장되어 있어야 정확한 격자 분석이 가능합니다.
    ...
    for t_id in [0, 1]:
        ...
        # 각 격자점마다 가장 가까운 팀 확인 (단순화된 KD-Tree 방식 활용 가능)
        # 본 코드에서는 시각적 리포트 구성을 위해 '누적 점유' 방식으로 가공
    
    # 2. 리포트 레이아웃 설정 (왼쪽: 팀0 히트맵 / 가운데: 점유 통계 / 오른쪽: 팀1 히트맵)
    fig = plt.figure(figsize=(20, 10))
    gs = GridSpec(1, 3, width_ratios=[1, 0.6, 1], wspace=0.3)
    ...
    ax0.imshow(pitch_img, extent=[-52.5, 52.5, -34, 34], aspect='auto')
    ...
    ax1.imshow(pitch_img, extent=[-52.5, 52.5, -34, 34], aspect='auto')
...
#===========================
    ...
    # 3. 최종 상태 업데이트
    #보로노이 최종 리포트 생성
    report_img_path = generate_final_report(full_coords_history, filename, ai_text=ai_commentary)
